...
parser.add_argument('--LE', type=int, default=1,
                    help='on graphs (0), on line expansion (1)')
...
# Load data
if args.LE == 0:
    adj, features, labels, idx_train, idx_val, idx_test = load_data(0, dataset=args.dataset)
else:
    adj, Pvp, features, labels, idx_train, idx_val, idx_test = load_data(1, dataset=args.dataset)
...
def train(model, epoch):
    ...
    optimizer.zero_grad()
    if args.LE == 1:
        output = model(features, adj, args.LE, Pvp)
    else:
        output = model(features, adj, args.LE)
